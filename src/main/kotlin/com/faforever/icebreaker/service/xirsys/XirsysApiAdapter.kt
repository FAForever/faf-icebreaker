...
class XirsysApiAdapter(
    ...
    private val globalXirsysApiClient: XirsysApiClient =
        buildXirsysApiClient(xirsysProperties.baseUrl())

    private val apiClientByRegion = mutableMapOf<XirsysRegion, XirsysApiClient>()

    private fun buildXirsysApiClient(baseUrl: String) =
    ...
    fun requestIceServers(
        ...
        runCatching {
            ...
            .onFailure {
                LOG.error(
                    "Failed to request ice servers for namespace=${xirsysProperties.channelNamespace()}, environment=${fafProperties.environment()}, channelName=$channelName",
                    it,
                )
            }
            .getOrThrow()
    ...
    private inline fun <reified T : Any> parseAndUnwrap(getResponse: () -> String): T {
        ...
        return try {
            when (val result = objectMapper.readValue<XirsysResponse<T>>(response)) {
                is XirsysResponse.Error ->
                    throw XirsysSpecifiedApiException(
